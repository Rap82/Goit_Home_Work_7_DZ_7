...
#===================== Робота списками, Рекурсія . ДЕКОДУВАННЯ =====================
...
# вона може приймати значення X, Y або Z. Значення з'являються у довільному порядку та зберігаємо ми їх 
...
# Стиснення можна досягти заміною групи повторюваних значень на одноразове значення та лічильник
...
# def decode ( data ) : # Функція приймає закодований за вказаним алгоритмом список (алгоритм схожий на архівуваня ) і повертає розкодований список значень .
#                     #   Функція працю рекурсивно .Тобто викликає сама себе.
#                     #  Алгоритм : Наш вхідний закодований список перевіряємо чи він не пустий. ( умова виходу з рекурсії )
#                     #  Якщо пустий повертаємо його . Якщо не пустий то переходимо на тіло коду нижче .
#                     #  Де будемо в циклі поточний перший елемент поточного списку *data добавляти в наш вихідний список *decoded_list стільки раз скільки буде вказувати поточний другий елемент поточного списку  *data
#                     #  Далі будемо формувати наступний поточний список *encoded_list зрізом з попереднього поточого списку *data але вже починаючи з другого елементу 
#                     #  Наш новий список *encoded_list для декодування але вже без перших двох елементів з поперенього *data [2:] передаємо в функцію рекурсивно виклакавши її і передавши в *data новий поточний список *encoded_list . 
#                     #  Тобто скожним рекурсивним викликом наш потчний список *data буде скорочуватись на два елементи а дані з списку який повертаємо з функції на кожному кроці рекурсії *decoded_list будуть залишатись в памяті з індеком ітерації рекурсії.
#                     #  Так буде тривати доти поки наш поточний список  *encoded_list не стане порожнім. Тоді на останьому кроці рекурсії наш поточний список *data отримає значення порожнього списку з *encoded_list і вийде з рекурсії .
#                     #  Після завершення рекурсії буде збирати з памяті всі значення з повернутих *decoded_list на кожному кроці рекурсивного виклику *decode ( encoded_list )   в зворотньому порядку як вони повертались з функції .
#                     #  Рекурсивне збирання : Спочатку *decoded_list запише отсанє не порожнє значення отримане з циклу *for i in range ( int ( data [1] ) )
#                     #  для нашого випадку це - *decoded_list = ['Z', 'Z']
#                     #  Далі верне з памяті  крок вищої ітерації . і після циклу *for i in range ( int ( data [1] ) ) в *decoded_list допише спереді ['Y', 'Y', 'Y'] і верне з функції вже список ['Y', 'Y', 'Y', 'Z', 'Z']
#                     #  Далі верне з памяті  крок щевищої ітерації . і після циклу *for i in range ( int ( data [1] ) ) в *decoded_list допише спереді ['X', 'X',] і верне з функції вже список ['Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z']
#                     #  На останьому найвищому кроці з циклу верне саме перше значення *for i in range ( int ( data [1] ) ) і в *decoded_list допише спереді ['X', 'X', 'X'] і верне з функції вже остаточний список  ['X', 'X', 'X', 'Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z'] 
#                     #  який і буде повернутий як  результат роботи функції *decode ( data ) .
#                     #  Формуэмо наш список на поверненя *decoded_list методом додавання(конкатенація) двох списків (A = B + C). 
#                     # Де B = Поточному значеню *decoded_list С = попередньому *decoded_list . А = нове потачне знячення *decoded_list . 
    
    
#     if data == [] :  # Перевіряє чи поточний запис в *data є порожнім списком. Якщо перше значення в *data буде порожнім списком то відразу поверне його . якщо ні то буде точкою виходу з рекурсії на останьому її кроці і з функції вже повернеться кінцеве значення *decoded_list
        
#         return data  # Повертаємо поточне значення *data з функції , якщо запускалась рекурсія то це точка виходу з неї ,і поверне  кінцеве значення *decoded_list
    
#     decoded_list = []  # Список в який будемо додавати всі значення повернуті з функції рекурсивно аж поки не сформуємо остаточний список який і повернем з основної функцї.

#     for i in range ( int ( data [1] ) ) : # Цикл в якому будем проходитись по нашому поточному значенні з  *data[0]( поточний перший елемент з поточного списку *decode ( encoded_list )  ) 
#                                         #    і будемо додавати в наш *decoded_list  значення *data[0] стільки раз скільки є в другому елементі потчого списку  data [1] - вказує скльки раз потрібно записати *data[0] на поточній ітерації рекурсії.


#         decoded_list.append ( data[0] )  # Добавляємо в циклі в *decoded_list потчне значення *data[0] на кожному кроці циклу до його завершення.

#     # print ( encoded_list )
    
#     encoded_list = data [2:]  # в *encoded_list записуємо зріз з другого елементу попереднього списку і всіма елементами що залишились. Тобто відкідаємо перші два значення з попереднього списку і вже новий список передаємо в нашу основну функцію
#                               #    Для виклику рекурсії *decode ( encoded_list )

#     decoded_list += decode ( encoded_list ) # Викликаємо рекурсвно функцію *decode ( encoded_list ) з переданим новим значенням *encoded_list  вже без перших двох попередніх значень і добавлємо в наш *decoded_list повернуте поточне значення.
#                                             # поки виконується умова рекурсивного виклик на кожному кроці наш список який передаємо рекурсивно буде скорочуваись на 2 елементи а дані повернути з функції будуть зберігатись в памняті. 
#                                             # як тільки закінчаться елементи в поточному списку який передаєм в рекурсивну функцію *decode ( encoded_list ). Тобто *encoded_list стане порожнім списком.
#                                             #  Спрацює умова виходу з рекурсії *if data == [] : і з памяті в зворотньому порядку почнуть повертатись всі повернуті значення *decoded_list і добавлятись з переді в наш список *decoded_list
#                                             #  Після повернення з памяті останього значення *decoded_list (це перше до рекурсії) наш *decoded_list буде мітисти всі потрібні елементи його і повертаєм з функції остаточно.
#                                             # Для нашого тестового значення *data = [ "X", 3, "Z", 2, "X", 2, "Y", 3, "Z", 2 ]
#                                             #  В кінці поверне ['X', 'X', 'X', 'Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z']
#     # print ( decoded_list ) # тестовий прінт для контрою занчень

#     return decoded_list  # Повернення *decoded_list  . Поки працює рекурсіє буде повертаи в память проміжні значення . як тільки рекурсія почне схлопуватись(спрацює точка виходу з рекурсії , внашому випадку умова *if data == []) повертне кінцеве значення всього списку.


# # decoded_list = [] 

# data = [ "X", 3, "Z", 2, "X", 2, "Y", 3, "Z", 2 ]  # Тестове значення 
# # data = []

# print ( decode ( data ) ) # принтимо результат для тестового значення *data . ['X', 'X', 'X', 'Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z']


# ================================ Звдання 17 / Task 17 ======================================

# Короткий алгоритм роботи рекурсійних функції.
# В функції яку потрібно викликати рекурсивно має бути точка виходу. Зазвичай це перевірка на пустий списко чи рядок , чи словник чи число рівне 0 . 
# При досягнені якого  ми маємо вийти з основної функції.
# Вихід з  функції коли відбувся хоча б один рекурсивний її виклик , буде точка виходу з рекурсії . Після того як він відбудеться , 
# функція рекурсії почне схлопуватись (виконуватись в зворотньому напрямку де останій елемент перед виходом опрацюється в тілі першим ,і якщо ми повертаємо щось з функції то це результат передасться у крок вище  )
# Передостанім другий і так поки не дійде то першого . Після того  виконається тіло коду з першим рекурсивним випаком функція поверне нам остаточний результат. 
# Всі проміжні дані утримуються в памяті під своїм номером і повертаються з рекурсії в зворотньому порядку.
# Тіло в функції потрібно прописати так щоб ми змінювали вхідні дані поки не досягнем точки виходу. Зазвичай скорочуємо їх так щоб на одному кроці відбувся потрібні маніпуляції з цими даними і дані які потрібно передати на повторне опрацювання мають не містити вже тих даних які були опрацьовані.
# Як тільки вихід з рекурсії відбудиться то функція почне збирати і обробляти результати в зворотньому порядку.
# Якщо не передбачити вихід з рекурсії або ж не прорахувати досягнення цього виходу в логіці коду то функція зациклиться що приведе переповнення памяті і наша програма зависне.


#===================== Робота списками, Рекурсія . КОДУВАННЯ =====================
...
# Коментар : Функція приймає олин аргумент список або стрічку яку потрібно закодувати і повертає один аргумент Що для списку що для стрічки .
# Список закодаваних символів. Де першим йде символ рядок а дрігий елементом йде ціле число. Для пустого списку чи стрічки має повернути пустий список.
#   

# ++++++++++++++++++++++++++++++++++  Код /Code +++++++++++++++++++++++++++++++++++++++++++++++++

# def encode_list ( data : list) :  # Допоміжна функція яка приймає один аргумент типу *list- список . 
#                                   # повертає закодований список згідно умов завдання. Також для закодування використовуємо рекорсивний виклки функції.
...
#     encoded_list = []  # Список в який будемо додавати всі значення повернуті з функції рекурсивно аж поки не сформуємо остаточний список який і повернем з основної функцї.
#     count = 0          # Лячильник який буде рахувати скільки підряд йде однакових символів . Його будемо використовувати для зрізу наступних елементів щоб передати в рекурсивну функцію
#                        # Також значення лічильника виступає наступним елементом з списку після елементу смиволу який кодуємо.    
#     list_count = []    # Список в який будемо заносити наші значення *count тип список що потім легкод додавати два списку між собою.

#     for element in data : # Цикл в якому проходимося по всіх елементах вхідного списку *data = ['X', 'X', 'X', 'Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z'] 
#                           # Рахуємо скільки однакових символів зустрічаються в списку підряд. Збільшуючи наш *count на одиницю коли виконається умова рівності.
        
#         if element == data [0] : # Умова рівності  Порівнюємо поточний елемент з циклу з перешим елементом списку переданим у *data .
#               count += 1  # лічильник збільшуємо на одиницю на кожній ітерації циклу поки виконується умова. Для вхідного списку символів "X" йде на початку 3 штуки підряд.
#                         #   Як тільки умова невиконається тобто зустрінимо на поточній ітерації символ нерівний першому з списку припинємо наш цикл операторм *break
#                         # Ціль було досягнуто всі одинакові смиволи які йдуть підряд ,на початку вхідного списку ,  були пораховані.
#                         #  Для нашого прикладу *count = 3 в основному тілі функцї 
#         else :
#              break
    
#     list_count.append ( count ) # додаємо наше значення *count в список , *list_count буде містити оди чисельний елемент - число 3 .
#                                 # *list_count = [3]. для кожного елемента *count буде свій і буде відповідити кількості однакових елементів які йдуть в списку підряд. 
        
#     encoded_list = list ( data [0] ) + list_count  # Додаємо в наш список який будемо повертати з функції обєднані значення Поточного першого елементу і кількості його зустрічань в писку підряд.
#                                                    # Для формування елементу списку  *encoded_list використовуємо додавання списків. 
#                                                    #  В результаті ми накожному кроці рекурсії будемо мати список з двох елементів. Сам поточний символ і його клькість підряд з вхідного списку.
#                                                    #  Для нашого прикладу на першому кроці список *encoded_list  ["X", 3]  , на другому - # ["Z", 2] , на третьому # ["X", 2] і так далі.
#      #print(encoded_list)
#     encoded_list_revers = data [ count: ]          # після того як ми отримали потрібну інформацію символ і його кількість . Формуємо новий список   *encoded_list_revers за допомогою різу попереднього списку.
#                                                    # Починаємо зріз з елемента на який вкаже поточне значення *count і до кінця списку . 
#                                                    #  *data [ count : ] - Де *data вхідний список .(буде змінюватись на кожній ітерації рекурсії на два елементи поки незалишиться жодного , прийме значення пустого списку. )
#                                                    #  *count - поточне значення лічильника яке вказує скільки вже елемнтів опрацьовано в тілі функції . Потрібно вирізати від *count і до кінця списку . Оскільки другий параметер невказаний це означає що до кінця .
       

#     encoded_list += encode_list  ( encoded_list_revers ) #  Сформований вище список *encoded_list_revers але вже без опрацьованих елементів передаємо в нашу функцію *encode_list ( data )- рекурсивний виклик. Але вже список який передаємо в *data
#                                                         #  Функція викличе сама себе і повторить всі кроки з початку тільки вже с поточними вхідними даними переданими з **encoded_list_revers . 
#                                                         #  так буде відбуватись доти доки в поточному списку який ми кожен раз скорочуємо на 2 елементи незалишиться жодного елементу.
#                                                         #  Тоді наш поточний список  *encoded_list_revers буде порожнім списком і спрацює умова виходу з функції *if data == [] :
#                                                         #  Після досягнення рекурції точки виходу вона почене повертати отрмані під час рекурсії дані в зворотньому порядку .
#                                                         #  Спочатку виконається тіло функції з отсанім результатом . Результат повернем на ітерацію вище .
#                                                         #  Далі виконається тіло з передостанім значенням і результат повернеться на ітерацію вище  і так поки не доберется до першого результату . Опрацює його і поверне вже з функції остаточний результат.
#                                                         #  Для нашого прикладу на нанайнищому рівні рекурсії поверне ["Z", 2] . на рівень вище ["Y", 3, "Z", 2]
#                                                         #  Ще на рівень вище [ "X", 2, "Y", 3, "Z", 2]
#                                                         #  На найвищому рівні ["X", 3, "Z", 2, "X", 2, "Y", 3, "Z", 2] - Цей результат вже і поверне з функції.
#     return encoded_list  # Повернення з функцію результату . На кожному кроці згортання рекурсії буде змінювати значення ( доповнюватись новими елементами з попереднього списку)
#                          # Коли доповнення зкінчаться (на останьму кроці згортання рекурсії) Поверне отсаточні дані вже з функції на зовні. 
     

# def encode_str ( data : str ) : # # Допоміжна функція яка приймає один аргумент типу *str -рядок  . 
#                                   # повертає закодований список згідно умов завдання. Також для закодування використовуємо рекорсивний виклки функції.
#                                   # Алгоритм такий самий як і для функції кодування списку *encode_list ( data : list) є дина для опрацювання даних застосовуємо методи роботи з рядками .
    
#     if data == '' :  # Перевіряє чи поточний запис в *data є порожнім рядком. Якщо перше значення в *data буде порожнім рядком то відразу поверне з функції порожній список ( як описано в умові задачі , порожній рядок повертати непотрібно) . якщо ні то буде точкою виходу з рекурсії на останьому її кроці і з функції вже повернеться кінцеве значення *decoded_list
        
#         return []  # Повертаємо поточне значення *data з функції , якщо запускалась рекурсія то це точка виходу з неї ,і поверне  кінцеве значення *encoded_list
    
#     encoded_str = []  # Список в який будемо додавати всі значення повернуті з функції рекурсивно аж поки не сформуємо остаточний список який і повернем з основної функцї.
#     count = 0         # Лічильник .

#     for element in data :  # # Цикл в якому проходимося по всіх елементах вхідного рядка *data  = "XXXZZXXYYYZZ"
        
#         if element == data [0] : # Порівнюємо поточний елемент з циклу з першим з рядка.
#               count += 1         # рахуємо кількість схожих смиволів з рядка піряд.
#         else :
#              break               # Виходимо з списку як тільки відбулось перше неспівпадіння ( невиконалась наша умова *if element == data [0])
        
#     encoded_str.append ( count ) # Формуємо список з одного елемента *count
        
#     encoded_list = list ( data [0] ) + encoded_str  # в *encoded_list записуємо поточне значення символа і його кількості . Для нашого прикладу буде ["X","3"] # ["Z","2"] і так далі.

#     #print(encoded_list)
#     encoded_str_revers = data [ count : ] # Формуємо список для повернення в функцію вже без опрацьованих елементів методом зрізу . 
#                                           # зріз робимо від поточного значення *count(каунт буде містити кількість опрацьованих елементів з попереднього списку) до кінця списку що залишився.

#     # encoded_str_revers = data [ count: ]
#     encoded_list += encode_str ( encoded_str_revers ) # Повертаємо наш обрізаний список *encoded_str_revers назад в нашу функцію (рекурсивний виклик) .Вона опрацює поточні елементи в тілі циклу і знову прийме в себе обрізаний вже на ці елементи список .
#                                                       #  і так поки в поточному списку незалишиться елементів. Тоді список буде пустий а пустий список умова виходу з рекурсії.
#                                                      #  коли точка виходу буде досягнута фінкція почне схлопуватись . Повертаючи з памяті зформовані значення для кожної ітерації рекурсії в зворотньому порядку.  
    
#     return encoded_list   # Повернення з функцію результату . На кожному кроці згортання рекурсії буде змінювати значення (доповнюватись новими елементами з попереднього списку)
#                          # Коли доповнення зкінчаться (на останьму кроці згортання рекурсії) Поверне отсаточні дані вже з функції на зовні. 


# def encode ( data  ) :  # основна функція . в якій будемо перевіряти вхідні дані *data на те чи це список чи рядок і викликати взалежності від типу даних відповідні допоміжні функції.
     
#      if type ( data ) == list : # Перевіряємо чи тип даних *data є даними типу список *list.
#                                 #Якщо так то викликаємо підфункцію для кодування списків *encode_list ( data ) і повертаємо з основної функції отриманий результат.
          
#           return encode_list ( data ) # Виклик підфункції  функції і одночас повернення з основної функції результату 

#      if type ( data ) == str : # Перевіряємо чи тип даних *data є даними типу рядок *str.
#                                 # Якщо так то  викликаємо підфункцію для кодування рядків *encode_str ( data  ) і повертаємо з основної функції отриманий результат.
          
#             return encode_str ( data  ) # Виклик підфункції  функції і одночас повернення з основної функції результату 

#      else :
#           print (f" Невірні вхідні дані - вихід !!! \nЗакодувати можна дані типу *list або *str ") # Додаткове опрацювання якщо будуть дані відміні від типу *list чи *str
#                                                     # Принтимо що такий тип даних неможе бути закодований і виходимо з функції


# # data = ['X', 'X', 'X', 'Z', 'Z', 'X', 'X', 'Y', 'Y', 'Y', 'Z', 'Z']  # Тестове значення # поверне  ["X", 3, "Z", 2, "X", 2, "Y", 3, "Z", 2]
# # data = []                     # Тестове значення # поверне [].
# data = "XXXZZXXYYYZZ"           # Тестове значення # поверне  ["X", 3, "Z", 2, "X", 2, "Y", 3, "Z", 2]
# # data = ''                       # Тестове значення # поверне [].
